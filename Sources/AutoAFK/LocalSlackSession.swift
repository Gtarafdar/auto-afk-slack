...
enum LocalSlackSession {
    enum ExtractError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .keychainKeyNotFound:
                return "Couldn't read the Slack Safe Storage key from Keychain."
            case .decryptFailed:
    ...
    static func readSession() throws -> (cookieHeader: String, teams: [LocalTeam]) {
        ...
        let cookieHeader = try readCookieHeader()
    ...
    private static func readCookieHeader() throws -> String {
        ...
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("autoafk-\(UUID().uuidString)", isDirectory: true)
    ...
    /// Reads the "Slack Safe Storage" key from the Keychain (one-time prompt).
